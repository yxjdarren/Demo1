...
namespace student
{
    public partial class frmCourse : Form
    {
        ...
        private void btnChange_Click(object sender, EventArgs e)
        {
            try
            {
                sql = "update Courses set Cname = '" + dataGView.CurrentRow.Cells[1].Value.ToString() + "',CreditHour='" + dataGView.CurrentRow.Cells[2].Value.ToString() + "'where Cno = '" + dataGView.CurrentRow.Cells[0].Value.ToString() + "'";
                con.OperateData(sql);
                SetBind();
            } catch {
                MessageBox.Show("不能作该操作！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
